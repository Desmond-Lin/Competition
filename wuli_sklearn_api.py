...
def f_eng(df, event_df, is_train=True):
    ...
    df['dis'] = np.sqrt(df['x_minus_xcmc'] ** 2 + df['y_minus_ycmc'] ** 2 + (df['t'] * 29.98) ** 2)

    # 按照公式，拟合簇射前峰面
    l = np.sin(df['thetamc']) * np.cos(df['phimc'])
    m = np.sin(df['thetamc']) * np.sin(df['phimc'])
    df['surface_fitting'] = df['t'] - l * df['x'] - m * df['y']

    # 原初粒子的能量和次级粒子电量的比例关系，min-max数据标准化放大差距
    ...
    # x_std, x_mean, x_skew, y_std, y_mean, y_skew, t_std, t_skew, q_quantile, q_min, q_skew, q_mean
...
# 小样本训练
train_df = train_df.iloc[300000:500000, :]
labels = labels[300000:500000]
...
best_score = clf.best_score_['valid_0']['score']
